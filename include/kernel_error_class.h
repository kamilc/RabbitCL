#ifndef KernelErrorClass_h
#define KernelErrorClass_h

#define KERNEL_ERROR_CLASS(kernel_name) \
template<typename T> \
class kernel_name \
{ \
public: \
    ...
    void compute_matrix(matrix<T>& in, matrix<T>& targets, scalar<T>& out);\
    static void run_matrix(matrix<T>& in, matrix<T>& targets, scalar<T>& out, std::size_t local_size, std::size_t global_size) \
    { \
        kernel_name kernel = kernel_name::instance(); \
        kernel.local_work_size(0, local_size); \
        kernel.global_work_size(0, global_size); \
        kernel.compute_matrix(in, targets, out); \
    } \
